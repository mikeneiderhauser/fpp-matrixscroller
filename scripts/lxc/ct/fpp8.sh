...
#   bash <(curl -fsSL https://raw.githubusercontent.com/mikeneiderhauser/fpp-matrixscroller/master/scripts/lxc/ct/fpp8.sh)
...
if [[ -z "${INSTALL_SCRIPT_LOCAL}" ]]; then
  curl -fsSL "${RAW}/install/fpp8-install.sh" -o /tmp/fpp8-install.sh
  INSTALL_SCRIPT_LOCAL="/tmp/fpp8-install.sh"
fi
