...
#   bash <(curl -fsSL https://raw.githubusercontent.com/mikeneiderhauser/fpp-matrixscroller/master/scripts/lxc/ct/fpp9.sh)
...
APP="FPP9"
FPPBRANCH="9.5.3"
var_tags="${var_tags:-media;iot;lighting}"
var_cpu="${var_cpu:-2}"
...
if [[ -z "${INSTALL_SCRIPT_LOCAL}" ]]; then
  curl -fsSL "${RAW}/install/fpp9-install.sh" -o /tmp/fpp9-install.sh
  INSTALL_SCRIPT_LOCAL="/tmp/fpp9-install.sh"
fi
...
function update_script() {
  ...
  msg_info "Checking FPP version"
  local current
  current="$(git -C /opt/fpp describe --tags 2>/dev/null || git -C /opt/fpp rev-parse --short HEAD)"
  msg_ok "Current: ${current}"

  msg_info "Pulling latest ${FPPBRANCH}"
  $STD git -C /opt/fpp fetch --tags
  $STD git -C /opt/fpp checkout "${FPPBRANCH}"
  $STD git -C /opt/fpp pull
  msg_ok "Updated to ${FPPBRANCH}"
