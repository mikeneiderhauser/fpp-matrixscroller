...
class PanelController:
    ...
    def start(self, message: str, mode: str, song_key: str = ""):
        ...
        with self._lock:
            ...
            # Skip restart if nothing changed (song key change triggers restart for new override params)
...
class MatrixScrollerDaemon:
    ...
    def _get_metadata(self, status: dict) -> dict:
        ...
        cs       = status.get("current_sequence", "") or ""
        fallback = _sanitize_text(os.path.splitext(os.path.basename(cs))[0]) if cs else ""
